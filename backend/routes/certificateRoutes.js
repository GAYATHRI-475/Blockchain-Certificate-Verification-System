...
import { uploadToIPFS } from "../utils/ipfs.js";
import Certificate from "../models/Certificate.js";
import Issuer from "../models/Issuer.js"; // ✅ NEW IMPORT
...
router.post("/update-tx", async (req, res) => {
  try {
    ...
    if (success) {
      await Certificate.updateOne({ certId }, { txHash });
    } else {
      await Certificate.updateOne(
        { certId },
        {
...
/*
-----------------------------------
VERIFY CERTIFICATE (WITH COLLEGE)
-----------------------------------
*/
router.get("/verify/:certId", async (req, res) => {
  try {
    ...
    const issuer = await Issuer.findOne({
      email: certificate.issuerEmail
    });

    const data = {
      ...certificate._doc,
      collegeName: issuer ? issuer.collegeName : "-"
    };
...
/*
-----------------------------------
GET SINGLE CERTIFICATE (WITH COLLEGE)
-----------------------------------
*/
router.get("/:certId", async (req, res) => {
  try {
